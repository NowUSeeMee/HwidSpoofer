...
void Menu::MainMenu() {
  ...
  ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(10, 20));
  {
    draw_list->AddRectFilled(
        ImVec2(p.x + 30.f, p.y + 100.f), ImVec2(p.x + 300.f, p.y + 385.f),
        ImColor(ImVec4(COLOUR(27.0f), COLOUR(29.0f), COLOUR(36.0f), 1.f)),
        5.0f);

    draw_list->AddRectFilled(
        ImVec2(p.x + 45.f, p.y + 155.f), ImVec2(p.x + 285.f, p.y + 195.f),
        ImColor(ImVec4(COLOUR(34.0f), COLOUR(36.0f), COLOUR(44.0f), 1.f)),
        7.0f);
    draw_list->AddRectFilled(
        ImVec2(p.x + 45.f, p.y + 210.f), ImVec2(p.x + 285.f, p.y + 250.f),
        ImColor(ImVec4(COLOUR(34.0f), COLOUR(36.0f), COLOUR(44.0f), 1.f)),
        7.0f);
    draw_list->AddRectFilled(
        ImVec2(p.x + 45.f, p.y + 265.f), ImVec2(p.x + 285.f, p.y + 305.f),
        ImColor(ImVec4(COLOUR(34.0f), COLOUR(36.0f), COLOUR(44.0f), 1.f)),
        7.0f);
    draw_list->AddRectFilled(
        ImVec2(p.x + 45.f, p.y + 320.f), ImVec2(p.x + 285.f, p.y + 360.f),
        ImColor(ImVec4(COLOUR(34.0f), COLOUR(36.0f), COLOUR(44.0f), 1.f)),
        7.0f);

    draw_list->AddRectFilled(
        ImVec2(p.x + 330.f, p.y + 100.f), ImVec2(p.x + 710.f, p.y + 185.f),
        ImColor(ImVec4(COLOUR(27.0f), COLOUR(29.0f), COLOUR(36.0f), 1.f)),
        5.0f);
    ...
    ImGui::Text(_xor_("What is NowUSeeme Software?").c_str());

    ImGui::SetCursorPosY(ImGui::GetCursorPosY() - 10);
    ImGui::SetCursorPosX(ImGui::GetCursorPosX() + 345);
    ImGui::Text(
        _xor_("NowUSeeme Software (HWID Changer) is the tool")
            .c_str());

    ImGui::SetCursorPosY(ImGui::GetCursorPosY() - 15);
    ImGui::SetCursorPosX(ImGui::GetCursorPosX() + 345);
    ImGui::Text(
        _xor_("that can change your HWID. Supports Win 10 & 11.").c_str());
    ...
    ImGui::Checkbox(_xor_(" Diskdrive").c_str(), &diskdrive);
    ImGui::SameLine();
    ImGui::Checkbox(_xor_(" Ram").c_str(), &ram);
    ImGui::SameLine();
    ImGui::Checkbox(_xor_(" Bios").c_str(), &bios);
    ImGui::SameLine();
    ImGui::Checkbox(_xor_(" Mac").c_str(), &mac);
    ImGui::SameLine();
    ...
    ImGui::Checkbox(_xor_(" Volume").c_str(), &volume);
    ImGui::SameLine();
    ...
    if (ImGui::Button("Spoof", ImVec2(375, 40))) {
      ...
      // Run all spoof operations in a single thread with auto-close on completion
      std::thread spoofThread([]() {
          ProcessKiller();
          VanguardDrivers();
          GayMacChanger();
          CleanProcess();     // Native registry API spoofing
          CleanCaches();      // Batch-based registry spoofing (ComputerName, hostname, MountedDevices, etc.)

          CleanFiveM();
          // Show quick success message then auto-close
          MessageBoxA(NULL, "Spoof completed!\n\nRestart your PC now.", "NowUSeeme Software", MB_OK | MB_ICONINFORMATION);
          ExitProcess(0);
      });
      spoofThread.detach();
    }
    ...
    ImGui::SetCursorPosX(ImGui::GetCursorPosX() + 55);
    ImGui::SetCursorPosY(ImGui::GetCursorPosY() + 21);
    std::string bboardText =
        stringer(_xor_("Baseboard: "), UserInfo::baseboardID);
